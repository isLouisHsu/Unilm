...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, history_states=None):
        if history_states is None:
            ...
            mixed_query_layer = self.query(hidden_states)           # (batch_size, curr_length, hidden_size)                Q来自当前输入，减少单向编码运算
            mixed_key_layer = self.key(x_states)                    # (batch_size, input_length + curr_length, hidden_size)
            mixed_value_layer = self.value(x_states)                # (batch_size, input_length + curr_length, hidden_size)

        query_layer = self.transpose_for_scores(mixed_query_layer)  # (batch_size, num_heads, curr_length, hidden_size)
        key_layer = self.transpose_for_scores(mixed_key_layer)      # (batch_size, num_heads, input_length + curr_length, hidden_size)
        value_layer = self.transpose_for_scores(mixed_value_layer)  # (batch_size, num_heads, input_length + curr_length, hidden_size)
        ...
        attention_probs = self.dropout(attention_probs)             # (batch_size, num_heads, curr_length, input_length + curr_length)
        ...
        return context_layer                                        # (batch_size, curr_length, hidden_size)
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, 
        ...
        if (prev_embedding is not None) and (prev_encoded_layers is not None):
            history_states = prev_embedding                     # 第一次是缓存的词向量，之后是每层隐层向量
...
class UnilmForSeq2Seq(UnilmPreTrainedModel):
    ...
    def forward(
        ...
        token_type_ids=None, 
        attention_mask=None, 
        masked_lm_labels=None,  # (batch_size, num_pos)
        masked_pos=None,        # (batch_size, num_pos)
        masked_weights=None,    # (batch_size, num_pos) 即num_pos中实际有值的mask
        num_tokens_a=None, 
        num_tokens_b=None
    ):
        ...
        sequence_output_masked = gather_seq_out_by_pos(
            sequence_output, masked_pos)                            # (batch_size, num_pos, hidden_size)
        prediction_scores_masked = self.cls(sequence_output_masked) # (batch_size, num_pos, vocab_size)
...
class UnilmForSeq2SeqDecode(UnilmPreTrainedModel):
    ...
    def forward(
        ...
        token_type_ids, # (batch_size, total_length), total_length = input_length + target_length
        position_ids,   # (batch_size, total_length)
        attention_mask  # (batch_size, total_length, total_length)
    ):
        ...
        total_length = token_type_ids.size(1)
        ...
        while next_pos < total_length:
            ...
            x_input_ids = torch.cat((curr_ids, mask_ids), dim=1)    # 第一次循环是(batch_size, input_length + 1)，之后是(batch_size, 1 + 1)

            curr_token_type_ids = token_type_ids[:, start_pos: next_pos + 1]                    # (batch_size, curr_length + 1)
            curr_attention_mask = attention_mask[:, start_pos: next_pos + 1, : next_pos + 1]    # (batch_size, curr_length + 1)
            curr_position_ids   = position_ids[:, start_pos: next_pos+1]                        # (batch_size, curr_length + 1)
            ...
            if prev_embedding is None:
                prev_embedding = new_embedding[:, : -1, :]          # (batch_size, curr_length, hidden_size)
            else:
            ...
            if prev_encoded_layers is None:
                prev_encoded_layers = [x[:, :-1, :] 
                    for x in new_encoded_layers]                    # List[(batch_size, curr_length, hidden_size)]
            else:
        ...
        return torch.cat(output_ids, dim=1)                         # (batch_size, target_length)
